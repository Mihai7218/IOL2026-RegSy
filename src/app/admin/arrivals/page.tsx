...
import { adminListAllArrivalsDetailed, AdminTransportRow, type AdminTeamRow } from '@/services/firebaseApi'

const columns: ColumnDef<AdminTransportRow>[] = [
  { accessorKey: 'datetime', header: 'Date/Time'},
  { accessorKey: 'location_OTH', header: 'Origin'},
...
export default function AdminArrivalsPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const data = await adminListAllArrivalsDetailed()
        if (mounted) setRows(data)
      } catch (err: any) {
        if (mounted) setError(err?.message ?? 'Unable to load arrivals')
      } finally {
